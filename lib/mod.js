...
customElements.define("overflow-observer-debug", class extends HTMLElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    if (name === "for") {
      if (this.#target) this.#target.removeEventListener("overflow:changed", this);
      this.#target = this.target;
      if (this.#target) this.#target.addEventListener("overflow:changed", this);
    }
  }
  ...
  handleEvent(event) {
    if (event.type === "overflow:changed")  {
      this.innerHTML = `
      for #${this.getAttribute("for")}: ${Array.from(event.detail).join(", ")}
      `
    }
  }
});
